...
> **Katastrophenprotokoll — GRATIS-Standard (Guardrail „kostenlos zuerst"):** Nicht
> `transcribe.py` (ElevenLabs Scribe, kostenpflichtig) nutzen, sondern den lokalen
> Adapter **`helpers/transcribe_local.py`** — faster-whisper (`small`, cpu/int8,
> dasselbe Modell wie `hoeren.py`). Schreibt dasselbe Scribe-JSON-Schema, das
> `pack_transcripts.py`/`render.py` erwarten; **kein `ELEVENLABS_API_KEY` nötig**.
> Aufruf: `python helpers/transcribe_local.py <video> --edit-dir <edit> --language de`.
> Nur wenn Diarisierung mehrerer Sprecher wirklich gebraucht wird → Scribe/ElevenLabs-MCP.
> Unsere Shorts sind Ein-Sprecher-VO → `speaker_id` immer `S0`.
